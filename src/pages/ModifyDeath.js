...
import {getModDeathRegz} from "../redux/apiCalls"
import authFetch from '../axios';

import { handleChange,clearFilters } from '../redux/modDeathSlice';
...
const {
  ...
  type,
  registerations,
} = useSelector((state) => state.modDeath);
useEffect(() => {
...
useEffect(() => {
  getModDeathRegz(dispatch, search,
          result_per_page,
          page,
          type
         )
...
const statusBodyTemplate2 = (rowData) => {
  return <Link  className={`btn btn-primary text-primary font-bold`} to={`/single-death/${rowData.Birth_Reg_ID}`} >
VIEW 
  </Link>
}
